    ...
    submitVote(option) {
      if (this._poll.anonymous) {
        this.$dialog({
          ...
          message: __('This poll is anonymous. Once you vote, you cannot retract your vote. You are voting for "{option}". Continue?', { option: option.title }),
          actions: [
            {
              label: __('Vote for "{option}"', { option: option.title }),
              variant: 'solid',
              onClick: (close) => {
  ...
  computed: {
    ...
    stopTime() {
      ...
      if (this.$dayjs().diff(timestamp, 'day') < 7) {
        return __('Ended {time}', { time: this.$dayjs(timestamp).fromNow() })
      }
      if (this.$dayjs().diff(timestamp, 'year') < 1) {
        return __('Ended at {time}', { time: this.$dayjs(timestamp).format('D MMM, h:mm A') })
      }
      return __('Ended at {time}', { time: this.$dayjs(timestamp).format('D MMM YYYY, h:mm A') })
    },
    _poll() {
