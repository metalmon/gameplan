...
class GameplanSearch(Search):
    ...
    def index_doc(self, doc):
        ...
        if doc.doctype == "GP Discussion":
            ...
            team = frappe.db.get_value(
                doc.reference_doctype, doc.reference_name, "team", cache=True
            )
            project = frappe.db.get_value(
                doc.reference_doctype, doc.reference_name, "project", cache=True
            )
    ...
    def get_records(self):
        ...
        for d in frappe.db.get_all(
            "GP Task",
            fields=["name", "title", "description", "modified", "project", "team"],
        ):
            d.doctype = "GP Task"
            records.append(d)
        ...
        for d in frappe.db.get_all(
            "GP Page",
            fields=["name", "title", "content", "modified", "project", "team"],
        ):
            d.doctype = "GP Page"
            records.append(d)
