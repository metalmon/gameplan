...
CONNECTION_STRING = (
    "host='localhost' dbname='discourse_db' user='postgres' password='qwe' port=5432"
)
...
def migrate_posts():
    ...
    for i, topic in enumerate(topics):
        if frappe.db.exists(
            "Discourse ID Map", {"discourse_table": "topics", "discourse_id": topic.id}
        ):
            continue
...
def save_image(src, filename=None, doc=None):
    ...
    if len(parts) == 2:
        ...
        filename = f'{filename or ""}-{hash}{extn}'.replace(" ", "-").replace(
            "%20", "-"
        )
...
def get_user(user_id):
    return frappe.db.get_value(
        "Discourse ID Map",
        {"discourse_table": "users", "discourse_id": user_id},
        "reference_name",
    )
...
def migrate_categories():
    ...
    for i, d in enumerate(categories):
        ...
        project_doc = frappe.get_doc(
            doctype="GP Project", title=project, team=team_doc.name
        ).insert()
