...
class GPProject(ManageMembersMixin, Archivable, Document):
    ...
    def as_dict(self, *args, **kwargs) -> dict:
        ...
        d.is_pinned = bool(
            frappe.db.exists(
                "GP Pinned Project", {"project": self.name, "user": frappe.session.user}
            )
        )
        return d
    ...
    def delete_group(self, group):
        ...
        if tasks > 0:
            frappe.throw(
                f"Group {group} cannot be deleted because it has {tasks} tasks"
            )
    ...
    def move_to_team(self, team):
        ...
        for doctype in ["GP Task", "GP Discussion"]:
            for name in frappe.db.get_all(
                doctype, {"project": self.name}, pluck="name"
            ):
    ...
    def remove_guest(self, email):
        name = frappe.db.get_value(
            "GP Guest Access", {"project": self.name, "user": email}
        )
        if name:
    ...
    def is_followed(self):
        return bool(
            frappe.db.exists(
                "GP Followed Project",
                {"project": self.name, "user": frappe.session.user},
            )
        )
    ...
    def follow(self):
        if not self.is_followed:
            frappe.get_doc(doctype="GP Followed Project", project=self.name).insert(
